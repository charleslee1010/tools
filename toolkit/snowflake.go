...
const (
	SnowflakeWorkerBits uint8 = 10 // 每台机器(节点)的ID位数 10位最大可以有2^10=1024个节点
	...
	SnowflakeWorkerMax   int64 = -1 ^ (-1 << SnowflakeWorkerBits) // 节点ID的最大值，用于防止溢出
	numberMax   int64 = -1 ^ (-1 << numberBits) // 同上，用来表示生成id序号的最大值
	timeShift   uint8 = SnowflakeWorkerBits + numberBits // 时间戳向左的偏移量
	SnowflakeWorkerShift uint8 = numberBits              // 节点ID向左的偏移量
...
type SnowflakeWorker struct {
	...
	SnowflakeWorkerId  int64      // 该节点的ID
	number    int64      // 当前毫秒已经生成的id序列号(从0开始累加) 1毫秒内最多生成4096个ID
}

// 实例化一个工作节点
func NewSnowflakeWorker(SnowflakeWorkerId int64) (*SnowflakeWorker, error) {
	// 要先检测SnowflakeWorkerId是否在上面定义的范围内
	if SnowflakeWorkerId < 0 || SnowflakeWorkerId > SnowflakeWorkerMax {
		return nil, errors.New("SnowflakeWorker ID excess of quantity")
	}
	// 生成一个新节点
	return &SnowflakeWorker{
		timestamp: 0,
		SnowflakeWorkerId:  SnowflakeWorkerId,
		number:    0,
	}, nil
}

func (w *SnowflakeWorker) GetIdString() string {
...
func (w *SnowflakeWorker) GetId() int64 {
	...
	ID := int64((now-epoch)<<timeShift | (w.SnowflakeWorkerId << SnowflakeWorkerShift) | (w.number))
	return ID
}