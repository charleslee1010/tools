...
type OutBoundServerHandler func(*SocketConnection)
...
type OutboundServer struct {
	...
	Handler OutBoundServerHandler
//	Conns chan SocketConnection
}

func (s *OutboundServer) SetHandler(handler OutBoundServerHandler) {
	s.Handler = handler
}
...
func (s *OutboundServer) Start() error {
	...
	go func() {
		for {
			...
			Warning("Got new connection from: %s", conn.OriginatorAddr())
