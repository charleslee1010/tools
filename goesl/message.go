...
//	Error("call ReadMIMEHeader")
	cmr, err := m.tr.ReadMIMEHeader()
//	Error("exit ReadMIMEHeader")
	if err != nil {     // && err.Error() != "EOF" {
		Debug(ECouldNotReadMIMEHeaders)
		Error(err.Error())
		Error(ECouldNotReadMIMEHeaders, err)
		return err
	}
	
//	fmt.Println("got message header", cmr)
//	for sls, slv := range cmr {
//		fmt.Println("[key]=", sls, "[value]=", slv)
//	}
...
//	Debug(fmt.Sprint(cmr))
//	Debug("Content length = " + cmr.Get("Content-Length"))
	if lv := cmr.Get("Content-Length"); lv != "" {
		l, err := strconv.Atoi(lv)

		if err != nil {
			Debug(EInvalidContentLength)
		...
		if actualLen, err := io.ReadFull(m.r, m.Body); err != nil {
			Debug(ECouldNotReadyBody)
			
			Error(ECouldNotReadyBody, err)
			return err
		} else {
//			fmt.Println("Body expected = ", l)
//			fmt.Println("Body len = ", actualLen)
			if l != actualLen {
				panic("ERROR ******************Body ")
			}			
		}
...
//		}
	case "text/event-json":
		...
		for k, v := range decoded {
			...
			default:
				//delete(m.Headers, k)
				Debug("Removed non-string property (%s)", k)
			}
		}
